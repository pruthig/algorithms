...
int remoteAccessInitNatTURN()
{
		...
		APP_LOG("REMOTEACCESS", LOG_DEBUG, "\n In remoteAccessInit........");
		APP_LOG("REMOTEACCESS", LOG_DEBUG, "\n MAC Address of plug-in device is %s", g_szWiFiMacAddress);
		strncpy(authInf->my_mac_add, g_szWiFiMacAddress, sizeof(authInf->my_mac_add)-1);
		APP_LOG("REMOTEACCESS", LOG_DEBUG, "\n Serial Number of plug-in device is %s", g_szSerialNo);
		strncpy(authInf->serno, g_szSerialNo, sizeof(authInf->serno)-1);
		...
		pInfo->nattrav_on_rx_data_handler = (void *)&remoteAccessServiceHandler;
...
void* remoteAccessInitThd(void *args) {
    ...
    tu_set_my_thread_name( __FUNCTION__ );
		...
		APP_LOG("REMOTEACCESS",LOG_DEBUG, "gpluginRemAccessEnable flag value is %d ---remoteAccessInitThd", gpluginRemAccessEnable);
#ifdef __RINIT_M__
		...
		if ((gpluginRemAccessEnable) && (gpluginNatInitialized == NATCL_NOT_INITIALIZED)) {
				while ((getCurrentClientState() !=  STATE_CONNECTED)) {
						...
						if (i < MAX_RETRY_COUNT) {
								APP_LOG("REMOTEACCESS",LOG_DEBUG, "remoteAccessInit in loop waiting for time syn and net connection %d", i);
				...
				strncpy(szRstPrm, g_szRestoreState, sizeof(szRstPrm)-1);
...
void* remoteAccessInitThdAg(void *args) {
		int retVal=0, i = PLUGIN_SUCCESS;
		char szRstPrm[MAX_RES_LEN];

		APP_LOG("REMOTEACCESS",LOG_DEBUG, "gpluginRemAccessEnable flag value is %d --- remoteAccessInitThdAg", gpluginRemAccessEnable);
		if ((gpluginRemAccessEnable) && (gpluginNatInitialized == NATCL_NOT_INITIALIZED)) {
				while ((getCurrentClientState() !=  STATE_CONNECTED) && (gNTPTimeSet !=1)) {
						pluginUsleep(MIN_NATCL_SLEEP*1000000);
						if (i < MAX_RETRY_COUNT) {
								APP_LOG("REMOTEACCESS",LOG_DEBUG, "remoteAccessInit in loop waiting for time syn and net connection %d", i);
						}else if (i > MAX_LOG_ACOUNT) {
...
#if 0
int remoteTSDomainResolve(char* turn_server_name) {
		char ipArr[MAX_IP_COUNT][SIZE_64B];
		int num = PLUGIN_SUCCESS, i = PLUGIN_SUCCESS;
		...
		{
				APP_LOG("REMOTEACCESS", LOG_DEBUG, "remoteTSDomainResolve g_turnServerEnvIPaddr%s\n", g_turnServerEnvIPaddr);
				snprintf(gturn_serverip, sizeof(gturn_serverip), "%s", g_turnServerEnvIPaddr);
				num = 1;
		}
		else
		{
				remoteParseDomainLkup(turn_server_name, ipArr, &num);
				for (i=PLUGIN_SUCCESS; i < num; i++) {
						APP_LOG("REMOTEACCESS", LOG_DEBUG, "TURN SERVER IP-%d-%d-%s-\n", num, i, *(ipArr+i));
				}
				if (num > PLUGIN_SUCCESS) {
						snprintf(gturn_serverip, sizeof(gturn_serverip), "%s", *(ipArr));
				}
		}
		return num;
}
