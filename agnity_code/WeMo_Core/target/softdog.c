...
#define TIMER_MARGIN	310		/* Default is 310 seconds */
...
{
    int retVal=0;
    retVal = mod_timer(&watchdog_ticktock, jiffies+(soft_margin*HZ));
    //printk(KERN_EMERG "softdog: softdog_keepalive soft_margin=[%d] retVal: %d\n", soft_margin, retVal);
    //return 0;
    return retVal;
}
	...
	unsigned int cmd, unsigned long arg)
{
	...
	case WDIOC_KEEPALIVE:
	    Ret = softdog_keepalive();
	    //printk(KERN_EMERG "softdog: WDIOC_KEEPALIVE softdog_keepalive: %d\n",Ret);
	    if(Ret == 1)
		return 0;
	    else
		return -EFAULT;
	case WDIOC_SETTIMEOUT:
		...
		return -EINVAL;
	    Ret = softdog_keepalive();
	    //printk(KERN_EMERG "softdog: WDIOC_SETTIMEOUT softdog_keepalive Return: %d\n",Ret);
	    if(Ret == 1)
		return 0;
	    else
		return -EFAULT;
