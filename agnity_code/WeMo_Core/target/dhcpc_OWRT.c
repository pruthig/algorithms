...
static int getNvramParameter(char *parameter, char paramVal[])
{
    const char *srFile = "/tmp/nvFile";
    char srCmd[BUFF_SIZE];
    FILE *pSrFile = 0x00;
    char szflag[BUFF_SIZE];
    char *var = NULL, *val = NULL, *p = NULL;
    int len = 0;

    memset(srCmd, 0x0, sizeof(srCmd));

    if(parameter) {
	sprintf(srCmd, "nvram_get %s > %s", parameter, srFile);
    } else {
    ...
    if (pSrFile == 0x00)
    {   
	printf("\nFile opening %s error", srFile);
	return 0;
    }   

    memset(szflag, 0x0, sizeof(szflag));

    while (fgets(szflag, sizeof(szflag), pSrFile))
    {
	if ((p = strstr(szflag, SERIALNUMBER))) {
	    var = strtok(szflag,"=");
	    val = strtok(NULL,"=");
	    break;
	}
...
unsigned long gDeviceXid=0;
static unsigned long device_xid(void)
{
    char arr[12];
    unsigned long xid=0;
    char paramVal[BUFF_SIZE];
    char *serial = paramVal;
    int ret = 0;

    memset(paramVal, 0x0, sizeof(paramVal));

    if(!gDeviceXid)
    {
	ret = getNvramParameter("SerialNumber", paramVal);

	if (ret == 0)
	{
	    printf("\nNULL device serial number, using random xid\n");
	    gDeviceXid = random_xid();
	}
	else
	{
	    ...
	    strcpy(arr, "0x");
	    strcat(arr, serial+7);
	    xid = strtoul(arr, (char **) NULL, 16);

	    printf("\nCreated device xid: %s, val: %x\n", arr, xid);
	    gDeviceXid = xid;
	}
    }
...
int udhcpc_main(int argc UNUSED_PARAM, char **argv)
{
	...
	fd_set rfds;
	char wemo_ipaddr[BUFF_SIZE];
	char *val = wemo_ipaddr;
...
/* The rest has variable bit positions, need to be clever */
	...
	/* see if we have an IP address stored on flash */
	ret = getNvramParameter("wemo_ipaddr", wemo_ipaddr);
	if(ret > 2 && val)
	{
	    struct in_addr tmp_addr;
	    inet_aton(val, &tmp_addr);
	    requested_ip = tmp_addr.s_addr;
	    state = REQUESTING;
	    xid = device_xid();
	    printf("Using stored IP address!!!\n");
	}
 ...
 * the configuration process..." */
				...
				timeout = lease_seconds / 2;
				{
					...
					/* save this IP on flash for future use */
					memset(wemo_ipaddr, 0x0, sizeof(wemo_ipaddr));
					sprintf(wemo_ipaddr, "nvram_set wemo_ipaddr %s", inet_ntoa(temp_addr));
				}
				requested_ip = packet.yiaddr;
			...
			if (*message == DHCPNAK) {
				...
				/* Unsetting the IP on flash */
				memset(wemo_ipaddr, 0x0, sizeof(wemo_ipaddr));
				sprintf(wemo_ipaddr, "nvram_set wemo_ipaddr");
				system(wemo_ipaddr);
