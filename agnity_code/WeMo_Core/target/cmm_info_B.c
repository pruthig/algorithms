...
#define	LINE_LEN	(4+65+20+23+9+8+7+3+HIDDEN_LINE_LEN)	// Channel+SSID+Bssid+Security+Signal+WiressMode+ExtCh+NetworkType
	...
	IN  UINT32		MsgLen)
{
	...
	else
	{
		INT idx = 0;
		sprintf(Ssid, "0x");
		for (idx = 0; (idx < 30) && (idx < pBss->SsidLen); idx++)
	...
	IN	RTMP_IOCTL_INPUT_STRUCT	*wrq)
{
	...
	TotalLen = sizeof(CHAR)*((MAX_LEN_OF_BSS_TABLE)*max_len) + SITE_SURVEY_HDR_LEN;
	...
	sprintf(msg,"%s","\n");
	sprintf(msg+strlen(msg),"%-4s%-65s%-20s%-23s%-9s%-8s%-7s%-3s%-3s\n",
	    "Ch", "SSID", "BSSID", "Security", "Signal(%)", "W-Mode", " ExtCH"," NT"," H");
	...
	/* Fill in space characters to complete SITE_SURVEY_HDR_LEN bytes in header */
 	//printk("msg: %s, len: %d\n",msg, strlen(msg));
	memset(msg+strlen(msg)-1, ' ', SITE_SURVEY_HDR_LEN - strlen(msg) - 1);

	msg[SITE_SURVEY_HDR_LEN-2]='\n';
	msg[SITE_SURVEY_HDR_LEN-1]='\0';
 	//printk("New length: %d\n", strlen(msg));
...
//		if((strlen(msg)+max_len ) >= IW_SCAN_MAX_DATA)
		if((strlen(msg)+SITE_SURVEY_HDR_LEN ) >= TotalLen)
			break;
