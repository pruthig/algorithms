    	...
    	if (ATE_ON(pAd))
    	{
			...
			if (IS_RT5350(pAd))
			{
				if ((pAd->ate.bAutoTxAlc == TRUE)
					&& ((pAd->ate.Mode == ATE_TXFRAME) || (pAd->ate.Mode == ATE_TXCONT)))
				{
					RT5350ATEAsicAdjustTxPower(pAd);
				}

				RT5350ATEAsicExtraPowerOverMAC(pAd);			
			}
...
#endif /* CONFIG_STA_SUPPORT */
					if (NdisEqualMemory(pEid->Octet, pAd->SpecOUI, pAd->SpecOUILength))
					{
						if (pBss->bMatchSpecOUI) // Save only the first matched vendor specific IE.
							break;
						if ((pEid->Len + 2) > MAX_CUSTOM_LEN)
						{
							pBss->SpecIE.IELen = 0;
						...
						pBss->bMatchSpecOUI = TRUE;
						pBss->SpecIE.IELen = pEid->Len;
						NdisMoveMemory(pBss->SpecIE.IE, pEid->Octet, pBss->SpecIE.IELen);
						break;
					}
					break;
#ifdef CONFIG_STA_SUPPORT
