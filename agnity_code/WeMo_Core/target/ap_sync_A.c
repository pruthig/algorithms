	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	for(apidx=0; apidx<pAd->ApCfg.BssidNum; apidx++)
	{
        ...
        {
                //printk("kkkkkkkkkkkkkkkkkkkkkkkkkk\n");
                ULONG ProbeReqTmpLen;
                UCHAR RalinkSpecificProbeIe[130];
                NdisZeroMemory(RalinkSpecificProbeIe, sizeof(RalinkSpecificProbeIe));
                RalinkSpecificProbeIe[0]=IE_VENDOR_SPECIFIC;
                RalinkSpecificProbeIe[1]=pAd->SpecProbeRspIELenth;
                NdisCopyMemory(&RalinkSpecificProbeIe[2], pAd->SpecProbeRspIE, pAd->SpecProbeRspIELenth);

                //hex_dump("RalinkSpecificBeaconIe",RalinkSpecificBeaconIe,130);
                MakeOutgoingFrame(pOutBuffer+FrameLen, &ProbeReqTmpLen,
                                                        pAd->SpecProbeRspIELenth+2, RalinkSpecificProbeIe,
                                                        END_OF_ARGS);
                //printk("BeaconTmpLen = %d\n",BeaconTmpLen);
                FrameLen += ProbeReqTmpLen;
...
VOID APScanTimeoutAction(
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	DBGPRINT(RT_DEBUG_TRACE, ("%s:: Ch=%d, ScanType=%d\n", __FUNCTION__, pAd->MlmeAux.Channel, pAd->MlmeAux.ScanType));

	if ((pAd->MlmeAux.ScanType == SCAN_CISCO_ACTIVE) ||
		(pAd->MlmeAux.ScanType == SCAN_CISCO_PASSIVE))
	{
		/* to end single channel scan */
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (MlmeScanReqSanity(pAd, Elem->Msg, Elem->MsgLen, &BssType, (PCHAR)Ssid, &SsidLen, &ScanType))
	{
		...
		if ((pAd->MlmeAux.ScanType == SCAN_CISCO_ACTIVE) ||
			(pAd->MlmeAux.ScanType == SCAN_CISCO_PASSIVE))
		{
			/* start single channel scan by staying at
		...
		if ((pAd->MlmeAux.ScanType == SCAN_CISCO_ACTIVE) ||
			(pAd->MlmeAux.ScanType == SCAN_CISCO_PASSIVE))
		{
			/* No need to suspend data Tx when scan at operating channel */
		}
		else
