...
unsigned long device_xid(void)
{
	...
	if(!gDeviceXid)
	{
		...
		if (NULL == (serial = nvram_get("cwf_serial_number")))
		{
		...
		{
			/* form the device xid as: 'A' followed by last 7 characters of serial number*/ 
...
//	}
	...
	if(nvram_get("wemo_ipaddr") == NULL)
		...
		inet_aton((nvram_get("wemo_ipaddr")), &tmp_addr);
		requested_ip = tmp_addr.s_addr;
		state = REQUESTING;
		timeout = now;
		packet_num = 0;
		xid = device_xid();
		LOG(LOG_INFO, "Using stored IP address!!!\n");
        }
...
//					bootp_cnt_ack = 0;
				  ...
				  write_dhcpc_msg(dhcpc_buf);	
					...
					sprintf(wemo_ipaddr, "%s", inet_ntoa(temp_addr));

					char *ptr=nvram_get("wemo_ipaddr");
					if(!ptr || strcmp(ptr, wemo_ipaddr))
					{
          					nvram_set("wemo_ipaddr", wemo_ipaddr);
						LOG(LOG_INFO, "\nSaving IP on flash\n");
					}
