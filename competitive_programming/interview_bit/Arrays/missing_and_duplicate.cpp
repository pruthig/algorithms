...
int main() {
	...
	for (int i = 0; i < n; ++i)
		actual_sum += (long long)A[i]; // ::accumulate(A.begin(), A.end(), 0);

	long long std_sum = ((long long)n*((long long)n + 1)) / 2;
	long long actual_square = 0;
	for (int i = 0; i < n; ++i)
		actual_square += (long long)A[i] * (long long)A[i];

	long long std_square = ((long long)n*((long long)n + 1)*(2 * (long long)n + 1)) / 6;

	long long diff_squares = actual_square - std_square;
	long long term_diff = actual_sum - std_sum;

	long long term_sum = (long long)diff_squares / term_diff;
	long long missing = (long long)(term_sum - term_diff) / 2;
	long long duplicate = (long long)(term_sum + term_diff) / 2;
	if (duplicate < 0 && missing < 0) {
		int temp = duplicate;
		duplicate = -missing;
		missing = -temp;
	}
	cout << "Duplicate and missing are: " << duplicate << " " << missing << endl;
}
